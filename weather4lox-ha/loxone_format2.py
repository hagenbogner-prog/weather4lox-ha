#!/usr/bin/env python3
"""Serialize the Local Weather Service Emulator format=2 response.

The Loxone format=2 response has a 29-column metadata header, one 10-column
station metadata line, and 19-column hourly forecast rows.
"""
...
def build_rows(core, forecast, query, diagnostic=False):
    ...
    for item in forecast:
        ...
        if len(row) != WEATHER_COLUMNS:
            raise RuntimeError(
                f"Forecast row has {len(row)} columns, expected {WEATHER_COLUMNS}"
            )
...
def validate_payload(core, header, station, rows, expected_rows, expected_picto=None):
    ...
    for index, row in enumerate(rows):
        ...
        try:
            ...
            pictocode = None
        if pictocode not in core.VALID_PICTOS or (
...
def make_payload(core, forecast, query, diagnostic=False):
    ...
    if not validation["ok"]:
        raise RuntimeError(
            "Loxone response validation failed: " + json.dumps(validation)
        )
